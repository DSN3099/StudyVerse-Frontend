...
const CreateCourse = () => {
  ...
  const postData = async (post) => {
    try {
      ...
      console.log(data)
      navigate(`/courseupload/${data._id}`)
    } catch (err) {
      console.log(err)
    }
  }
      ...
      <form onSubmit={handleSubmit} className='w-full' id='form'>
        <div className='flex flex-col w-full gap-2 p-5 items-center'>
          <div className='flex w-1/2 justify-between gap-5'>
            <TextField autoComplete='false' required sx={{ width: "50%" }} value={courseData.title} onChange={(e) => setCourseData({ ...courseData, title: e.target.value })} size='small' type={"text"} label="Title" />
            <TextField autoComplete='false' required sx={{ width: "50%" }} value={courseData.category}
          ...
          <div className='flex w-1/2 justify-between gap-5'>
            ...
            <TextField autoComplete='false' sx={{ width: "50%" }} size='small' type={"text"} label="Level" value={courseData.level} onChange={(e) => setCourseData({ ...courseData, level: e.target.value })} />
          </div>
