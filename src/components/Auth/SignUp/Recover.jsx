...
const Recover = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { data } = await axios.post('http://localhost:5000/api/auth/recover', states, { withCredentials: true })
      console.log(data)
  ...
  return (
    <div class=' flex items-center gap-8 mt-5 flex-col sm:w-full '>
      {error &&
      ...
      <div class=' flex justify-center w-full h-full sm:w-full sm:px-10 '>
        <div class=' flex flex-col w-1/2  bg-white gap-3 sm:w-full'>
          <div class='flex  '>
            <span class='font-bold text-3xl'>Account Recovery</span>
          </div>
            ...
            <form onSubmit={handleSubmit} class='flex flex-col gap-4 '>
              ...
              <div>
                <button type='submit' class='flex items-center justify-center  gap-2 bg-blue-600 w-full p-2.5 rounded-md'>
                  <span class='text-2xl text-white'>Recover Account</span>
                </button>
              </div>
            </form>
